...
{
    var casURL = 'http://' + this.hostname + ':' + this.port + this.base_path;
...
CAS.prototype.logout = function(req, res, returnUrl, doRedirect)
{
    ...
    var redirectURL = 'http://' + this.hostname + ':' + this.port + this.base_path + logout_path;
    res.writeHead(307, {'Location' : redirectURL});
      ...
      if (cas_version < 2.0) {
        ...
        if (sections.length >= 1) {
          if (sections[0] == 'no') {
            ...
            return;
          } else if (sections[0].trim() == 'yes' &&  sections.length >= 2) {
...
CAS.prototype.startPgtServer = function(key, cert, ca, callbackHost, callbackPort, proxyPort) 
{
    ...
    this.pgt_url = 'http://' + callbackHost + ':' + callbackPort + '/';
...
CAS.prototype.proxiedRequest = function(pgtIOU, options, callback) 
{
    ...
    this.getProxyTicket(pgtIOU, targetService, function(err, pt) {
        ...
        var path = options.path || targetService.replace(/^http?:\/\/[^\/]+/, '');
        if (path.match(/[&?]/)) {
